...
void MtgCards::loadSetFromFile(QString setFileName) {
    ...
    for(QJsonValueRef jsonCardRef: jsonCards) {
		...
		int mtgaId = getMtgaId(setCode, number);
        Card *card = new Card(mtgaId, number, name, setCode);
		cards[mtgaId] = card;
	}
...
int MtgCards::getMtgaId(QString setCode, QString cardNumber)
{
	return mtgaIds[setCode][cardNumber];
}
